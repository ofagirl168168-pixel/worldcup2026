  ...
  async function _showStarterCompleteToast(n) {
    ...
    t.querySelector('.mt-starter-primary').addEventListener('click', async () => {
      ...
      if (action === 'coach') {
        ...
        _spotlightTab('train', '[data-feed-mode="normal"]', '點這裡開始訓練', async () => {
          // 等 picker 開
      ...
      <div class="mt-home-tab">
        ...
        <div class="mt-home-scene" id="mt-home-scene">
          ...
          <div class="mt-home-ground" id="mt-home-ground">
            ${stadiumLv >= 5 ? _renderPitchAds(stadiumLv, team) : ''}
...
<div class="mt-home-fountain">
  ...
  // z-index 在 CSS 對每 variant 設好（依 bottom% 對齊球員 z = 100 + y*100）
...
<svg viewBox="0 0 200 110" preserveAspectRatio="xMidYMax meet" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="sky${lv}" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="${isMaxed ? '#ffb988' : '#7eb6e0'}"/>
      <stop offset="60%" stop-color="${isMaxed ? '#ffd9b0' : '#b3d7eb'}"/>
      <stop offset="100%" stop-color="${isMaxed ? '#fff0c8' : '#d4e7f3'}"/>
    </linearGradient>
    <linearGradient id="grass${lv}" x1="0" y1="0" x2="0" y2="1">
      ...
      <div class="mt-stadium-upgrade-card">
        ...
        <div class="mt-stadium-up-compare">
          ...
          <div class="mt-stadium-up-side">
            <div class="mt-stadium-up-mini">${_stadiumPreviewSvg(lv)}</div>
          ...
          <div class="mt-stadium-up-side ${isMaxed ? 'is-maxed' : 'is-next'}">
            <div class="mt-stadium-up-mini">${_stadiumPreviewSvg(isMaxed ? lv : lv + 1)}</div>
    ...
    content.innerHTML = `
      <div class="mt-match-map" data-tier="${tier}">
        ...
        <div class="mt-season-wrap" data-tier="${tier}">
          <!-- 賽季進度 HUD：聯賽名 + 戰績 + 7 勝升甲組目標線 -->
          <div class="mt-season-hud">
            <!-- 聯賽名稱（HUD 內、靠頂、置中）-->
            <div class="mt-season-title-inner">
              🏆 <b>${escapeHtml(tierName)}</b>
              <span class="mt-season-title-meta">Tier ${tier}・賽季 ${seasonNum}</span>
            </div>
  ...
  async function _renderMatchBanner(content, mascot, team, tier = 1, matchIdx = 0) {
    ...
    // 走向約定：依專案的 walkingFullBody — row 0=down, 1=left, 2=right, 3=up,
    //   4=kick, 5=cheer, 6=hurt/frustration, 7=tackle, 8=GK dive
    ...
    const tick = () => {
      if (phase === 'run') {
        ...
        if (heroX >= goalX() - frameW - 26) {
          ...
          // 對手見射門 → 鏟球防守
          try { window.__mtMatchSetOppPose?.('tackle'); } catch (e) {}
          // 球加速飛入網
          ...
          setTimeout(() => {
            ...
            // 對手懊惱
            try { window.__mtMatchSetOppPose?.('frustrated'); } catch (e) {}
            setTimeout(() => {
              // 重置
              heroX = -frameW;
              frame = 1; frameTick = 0;
              ...
              ballEl.style.transform = 'rotate(0deg)';
              // 對手回 idle
              try { window.__mtMatchSetOppPose?.('idle'); } catch (e) {}
              phase = 'run';
            }, 900);
          }, 460);
        }
      ...
      <div class="mt-profile-card">
        ...
        <div class="mt-train-info-section">
          ...
          <div class="mt-train-info-a">
            ...
            <b>❤️ 鬥志</b> ← 擂台投票（首投 +10 / 之後 +2）+ 留言（每則 +3）+ 麥迪挑戰賽（每場 +5）<br>
            <b>💡 靈感</b> ← 預測比賽（每場 +5）+ 看文章（+3）+ 射門挑戰（每場 +5）<br><br>
